...
function getPeer(targetUserId) {
  ...
  state.peers.set(targetUserId, { pc, remoteStream });
...
function syncUsers(users = []) {
  ...
  for (const [id, user] of state.users.entries()) {
    ...
    // Existing user initiates to newly seen user to avoid glare.
    if (!peerExists && user.joined_at >= (me?.joined_at || 0)) {
